...
![Facade Pattern](./images/facadeless.jpg)

The client knows the elements of the module and orchestrates the work.
...
We don't want the client code to interact with all services just to make an order, so we provide a nice and clean API with Facade.
...
All services methods are hidden behind the facade - making an order is much simpler now.
...
by adding more methods to Facades.
