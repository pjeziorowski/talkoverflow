...
Let's suppose that the client sent this query:
```
query {
  wish(id: 123) {
    title
...
It doesn't know how! You need to connect the schema to some executable code that is able to get the required data or execute appropriate business logic.
...
One of the core Vert.x components is the [Event Bus](https://vertx.io/docs/apidocs/io/vertx/core/eventbus/EventBus.html). Event Bus can be used as a communication channel between various parts of the application.
...
You don't need to implement anything (except for the logic that handles the data we receive from the other application, of course).
...
Let's go through one of the overridden methods:
...
This part is marked in the image below:

![Database Event Bus Request](./images/4.png)

The gRPC server uses the event bus to request wish data. On the second side of the event bus there is a Database Verticle
